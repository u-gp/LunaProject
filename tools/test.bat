...
echo ========================================
echo  Luna - Unit Tests (Vitest + cargo)
echo ========================================
echo.
...
echo [INFO] フロントユニットテストを実行します...
call npm test
set "EXIT_CODE=%ERRORLEVEL%"
if not "%EXIT_CODE%"=="0" (
  echo [ERROR] フロントテストに失敗しました。exit code=%EXIT_CODE%
  exit /b %EXIT_CODE%
)

echo [INFO] Rust ユニットテストを実行します...
pushd "src-tauri"
call cargo test
set "EXIT_CODE=%ERRORLEVEL%"
popd
if not "%EXIT_CODE%"=="0" (
  echo [ERROR] Rust テストに失敗しました。exit code=%EXIT_CODE%
  exit /b %EXIT_CODE%
)
